...
doesGangExist = function(name) {
  return Meteor.users.find({username: name.toLowerCase()}).count() > 0;
}
...
countMessagesAfterTime = function(time) {
  if (time == null) {
    return Messages.find({gangId: getGangId()}).count();
  }  
  var timePlus = new Date(time.getTime() + 1000);
  return Messages.find({gangId: getGangId(), time: {$gt: timePlus}}).count();
}
